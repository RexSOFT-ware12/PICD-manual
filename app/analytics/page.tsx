...
function BarChart({ rows, title }: { rows: AnalyticsBucket[]; title: string }) {
  ...
  return <div className="rounded-2xl border border-line bg-white p-5 shadow-sm">
    ...
    <div className="flex h-56 items-end gap-1.5 overflow-hidden border-b border-line px-1">
...
export default function AnalyticsPage() {
  const [preset,setPreset]=useState("week"); const [from,setFrom]=useState(""); const [to,setTo]=useState(""); const [data,setData]=useState<AnalyticsResponse|null>(null); const [loading,setLoading]=useState(false); const [error,setError]=useState<string|null>(null);
  const load=useCallback(async()=>{setLoading(true);setError(null);try{const range=preset==="custom"?{from:from||undefined,to:to||undefined}:rangeForPreset(preset);setData(await fetchAnalytics(loadSettings(),range.from,range.to));}catch(e){setError(e instanceof ApiError?e.message:"Could not load analytics.");}finally{setLoading(false);}},[preset,from,to]);
  useEffect(()=>{load()},[load]);
  return <main className="min-h-screen animate-app-enter bg-paper px-4 py-6 sm:px-8"><div className="mx-auto max-w-7xl">
    <header className="mb-6 flex flex-wrap items-end justify-between gap-4"><div><a href="/" className="text-[11px] font-medium text-blueprint hover:underline">← Pipeline board</a><h1 className="mt-2 font-display text-3xl font-semibold">Scan analytics</h1><p className="mt-1 text-sm text-ink/45">Compare incoming scans by day, week and month.</p></div><button onClick={()=>data&&exportCsv(data)} disabled={!data} className="rounded-xl bg-blueprint px-4 py-2.5 text-xs font-semibold text-paper shadow-sm transition hover:-translate-y-0.5 hover:shadow-md disabled:opacity-40">Export Excel-compatible CSV</button></header>
    ...
    {loading&&!data?<div className="grid gap-4 lg:grid-cols-2"><div className="h-72 animate-pulse rounded-2xl bg-ink/5"/><div className="h-72 animate-pulse rounded-2xl bg-ink/5"/></div>:data&&<>
      ...
      <div className="grid gap-4 lg:grid-cols-[1.6fr_1fr]"><BarChart rows={data.daily} title="Daily incoming scans"/><Donut data={data.counts}/></div>
      <div className="mt-4 grid gap-4 lg:grid-cols-2"><LineChart rows={data.weekly}/><BarChart rows={data.monthly} title="Monthly incoming scans"/></div>
      <p className="mt-5 text-[11px] text-ink/35">Range: {data.from} → {data.to} · {data.total} incoming scans. Export includes daily, weekly and monthly breakdowns.</p>
    </>}
  </div></main>;
}
