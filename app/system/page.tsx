...
export default function SystemPage({ section = "overview" }: { section?: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setState({ ...a, features: a?.features && typeof a.features === "object" ? a.features : {}, config: a?.config && typeof a.config === "object" ? a.config : {} });
      setHealth(b);
      setEmail({ ...c, recipients: Array.isArray(c?.recipients) ? c.recipients.filter((x): x is string => typeof x === "string") : [] });
      setEmailTemplates((c as any)?.templates && typeof (c as any).templates === "object" ? (c as any).templates : {});
      setClientEstimate(d);
      setBodyAnalyzer(e);
      setOperational({
        ...f,
        ...
        access: f?.access && typeof f.access === "object" ? f.access : {},
  ...
  const saveEmail = async () => {
    if (!email) return;
    const recipients = (Array.isArray(email?.recipients) ? email.recipients : []).map(x => x.trim()).filter(Boolean);
    setEmailBusy(true); setEmailMessage(null);
    try { setEmail(await saveEmailSettings(loadSettings(), { ...email, recipients, templates: emailTemplates } as any)); setEmailMessage("Email settings saved."); }
  ...
  const removeRecipient = async (index: number) => {
    ...
    const next = { ...email, recipients: (Array.isArray(email?.recipients) ? email.recipients : []).filter((_, j) => j !== index) };
    setEmail(next);
    setEmailBusy(true);
    setEmailMessage(null);
    try {
      const recipients = (Array.isArray(next.recipients) ? next.recipients : []).map(x => x.trim()).filter(Boolean);
...
<Card className="overflow-hidden">
              ...
              {email && <div className="grid grid-cols-[minmax(0,1fr)_250px] gap-6 px-5 py-5">
                <div className="min-w-0 space-y-5">
                  ...
                  <div>
                    ...
                    <div className="rounded-xl border border-line bg-paper/40">
                      <div className="max-h-[220px] overflow-y-auto p-1.5 scrollbar-thin">
                        {(Array.isArray(email.recipients) ? email.recipients : []).map((r, i) => <div key={`${i}-${r}`} className="group flex items-center gap-2 border-b border-line/70 px-2 py-1.5 last:border-0">
                  ...
                  <div className="rounded-xl border border-line bg-paper/35 p-4"><div className="mb-3"><p className="text-xs font-semibold">Email templates</p><p className="mt-0.5 text-[10px] leading-4 text-ink/35">Customize the subject and message used for each notification. Supported tokens: <code>{`{title}`}</code>, <code>{`{message}`}</code>, <code>{`{scan_id}`}</code>, <code>{`{time}`}</code>.</p></div><div className="space-y-3">{events.map(([k,label])=>{const t=emailTemplates[k] || {subject:`[PICD Manual] {title}`,body:`{title}

{message}

Scan ID: {scan_id}
Time: {time}`};return <div key={k} className="rounded-xl border border-line bg-white p-3"><p className="mb-2 text-[10px] font-bold uppercase tracking-wider text-ink/35">{label}</p><input value={t.subject} onChange={e=>setEmailTemplates(v=>({...v,[k]:{...t,subject:e.target.value}}))} placeholder="Subject" className="w-full rounded-lg border border-line px-3 py-2 text-xs"/><textarea value={t.body} onChange={e=>setEmailTemplates(v=>({...v,[k]:{...t,body:e.target.value}}))} className="mt-2 min-h-20 w-full rounded-lg border border-line px-3 py-2 font-mono text-[10px] leading-4"/></div>})}</div></div>
...
<div className="flex flex-wrap items-center gap-2 border-t border-line pt-4">
                    ...
                    <span role="status" className={`rounded-lg px-2.5 py-1.5 text-[10px] font-semibold ${emailMessage ? (emailMessage.includes("failed") || emailMessage.includes("Could") ? "bg-brick/10 text-brick" : "bg-sage/10 text-sage") : "text-ink/40"}`}>{emailMessage || ""}</span>
                  </div>
                </div>
...
<div id="system-sound" className="scroll-mt-5"><Card className="p-5">
      ...
      case "general": return (<>
<div id="system-general" className="scroll-mt-5"><Card className="p-5"><div className="mb-4 flex items-start justify-between"><div><h2 className="font-display font-semibold">General settings</h2><p className="mt-1 text-xs text-ink/40">Backend connection used by the authenticated desktop dashboard.</p></div><div className="text-right"><p className="mb-2 text-[9px] font-bold uppercase tracking-wider text-ink/30">Backend URL</p><p className="mb-2 max-w-[430px] break-all font-mono text-[10px] text-ink/55">{settings.baseUrl || "Not configured"}</p><ConnectionSettingsPanel settings={settings} status={connectionStatus} onSave={save}/></div></div></Card></div>
...
<div id="system-operations" className="scroll-mt-5"><Card className="overflow-hidden"><div className="border-b border-line px-5 py-5 flex items-start justify-between gap-5"><div><h2 className="font-display font-semibold">Operations & calibration controls</h2><p className="mt-1 max-w-3xl text-xs leading-5 text-ink/40">Business and operational thresholds live here instead of being buried in Python. Security and implementation safeguards remain code-controlled.</p></div><div className="flex gap-2"><button onClick={resetOperational} className="rounded-lg border border-line bg-white px-3 py-2 text-[11px] font-semibold text-ink/55">Restore defaults</button><button onClick={saveOperational} disabled={!operational} className="rounded-lg bg-blueprint px-3 py-2 text-[11px] font-semibold text-paper disabled:opacity-40">Save operations</button></div></div>{operational && <div className="p-5 space-y-6">
              ...
              <div className="rounded-xl border border-blueprint/15 bg-blueprint/[0.025] p-4">
                <div className="flex items-start justify-between gap-5">
                  <div><h3 className="text-xs font-semibold">Creative processing engine</h3><p className="mt-1 max-w-2xl text-[10px] leading-4 text-ink/40">Choose where the Photoshop/Illustrator stages run. Web uses PICD Photo Workspace → Artwork in the browser. DAZ Studio and the existing body-analysis stage remain on the desktop processing worker.</p></div>
                  <button type="button" role="switch" aria-checked={operational.processing.editor_mode === "web"} onClick={() => setOperational({...operational, processing:{...operational.processing, editor_mode:operational.processing.editor_mode === "web" ? "desktop" : "web"}})} className={`relative flex h-10 w-[250px] items-center rounded-full border px-1 transition ${operational.processing.editor_mode === "web" ? "border-blueprint bg-blueprint/10" : "border-line bg-paper"}`}>
                    <span className={`h-8 w-8 rounded-full bg-white shadow-sm transition-transform ${operational.processing.editor_mode === "web" ? "translate-x-[208px]" : "translate-x-0"}`} />
                    <span className={`absolute inset-0 flex items-center justify-center text-[11px] font-semibold ${operational.processing.editor_mode === "web" ? "text-blueprint" : "text-ink/55"}`}>{operational.processing.editor_mode === "web" ? "Web Photo Workspace + Artwork" : "Desktop Photoshop + Illustrator"}</span>
                  </button>
                </div>
                <div className="mt-3 grid grid-cols-2 gap-3 text-[10px] leading-4 text-ink/45">
                  <div className="rounded-lg border border-line bg-white px-3 py-2"><b className="text-ink/70">Desktop</b><br/>Photoshop → Illustrator → body analysis → DAZ</div>
                  <div className="rounded-lg border border-line bg-white px-3 py-2"><b className="text-ink/70">Web</b><br/>Photo Workspace → Artwork → body analysis → desktop DAZ</div>
                </div>
              </div>
              <div className="rounded-xl border border-blueprint/15 bg-blueprint/[0.025] p-4"><div className="flex items-start justify-between gap-4"><div><h3 className="text-xs font-semibold">Gemini AI image review</h3><p className="mt-1 text-[10px] leading-4 text-ink/40">Adds a second visual review alongside OpenCV and pose checks. The API key stays on the backend and is never sent to the browser.</p></div><label className="flex shrink-0 items-center gap-2 text-[11px] font-semibold text-ink/55"><input type="checkbox" checked={operational.image_qc.gemini_enabled} onChange={e=>setOperational({...operational,image_qc:{...operational.image_qc,gemini_enabled:e.target.checked}})} className="h-4 w-4 accent-blueprint"/> Enabled</label></div><div className="mt-3 grid grid-cols-2 gap-3"><label className="text-[11px] font-semibold text-ink/55">Gemini model<select value={operational.image_qc.gemini_model} onChange={e=>setOperational({...operational,image_qc:{...operational.image_qc,gemini_model:e.target.value}})} className="mt-1.5 h-10 w-full rounded-lg border border-line bg-white px-3 text-xs outline-none focus:border-blueprint/50"><option value="gemini-3.8-flash">Gemini 3.8 Flash</option><option value="gemini-2.5-flash">Gemini 2.5 Flash</option></select></label><div className="flex items-end rounded-lg border border-line bg-white px-3 py-2 text-[10px] leading-4 text-ink/40">Set <code className="mx-1 rounded bg-ink/5 px-1">GEMINI_API_KEY</code> in the backend environment. Never paste the key into dashboard settings.</div></div></div>
...
<div id="system-estimate" className="scroll-mt-5"><Card className="overflow-hidden">
      ...
      case "body": return (<>
<div id="system-body" className="scroll-mt-5"><Card className="overflow-hidden">
...
<div id="system-database" className="scroll-mt-5"><Card className="p-5"><h2 className="font-display font-semibold">Database</h2><p className="mt-1 text-xs text-ink/40">MongoDB powers persistent scan, alert and authentication storage. Connection secrets remain on the backend.</p></Card></div>
      ...
      case "logs": return (<>
<div id="system-logs" className="scroll-mt-5"><Card className="p-5"><div className="flex items-center justify-between"><div><h2 className="font-display font-semibold">Diagnostics & logs</h2><p className="mt-1 text-xs text-ink/40">Run live checks against API, MongoDB, queue, worker and scan storage.</p></div><button disabled={busy || !settings.baseUrl} onClick={diagnostics} className="rounded-lg border border-line px-3 py-2 text-[11px] font-semibold text-ink/60 disabled:opacity-40">{busy ? "Running checks…" : "Run full diagnostic"}</button></div>{busy && <div className="mt-4 rounded-xl border border-blueprint/15 bg-blueprint/5 px-4 py-3"><div className="flex items-center gap-3"><span className="h-4 w-4 animate-spin rounded-full border-2 border-blueprint/20 border-t-blueprint"/><div><p className="text-xs font-semibold text-blueprint">Live diagnostic in progress</p><p className="mt-0.5 text-[10px] text-blueprint/55">Checking each service and recording the result.</p></div></div><div className="mt-4 overflow-hidden rounded-xl border border-blueprint/10 bg-white/70 p-3"><div className="flex items-center justify-between text-[9px] font-bold uppercase tracking-[.15em] text-blueprint/55"><span>Diagnostic sweep</span><span className="animate-pulse">LIVE</span></div><div className="mt-3 flex gap-1">{Array.from({length:12}).map((_,i)=><span key={i} className="h-1.5 flex-1 overflow-hidden rounded-full bg-blueprint/10"><span className="block h-full w-full origin-left animate-[diagnostic-scan_1.2s_ease-in-out_infinite] rounded-full bg-blueprint/45" style={{animationDelay:`${i*70}ms`}}/></span>)}</div></div></div>}{diag && <div className="mt-4 grid grid-cols-2 gap-2">{diag.map((x, i) => <div key={x.name} className={`rounded-xl border px-3 py-3 transition-all duration-300 ${x.status === "checking" ? "border-blueprint/20 bg-blueprint/5" : x.ok ? "border-sage/15 bg-sage/10" : "border-brick/15 bg-brick/10"}`}><div className={`flex items-center gap-2 text-xs font-semibold ${x.status === "checking" ? "text-blueprint" : x.ok ? "text-sage" : "text-brick"}`}>{x.status === "checking" ? <span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-blueprint/20 border-t-blueprint"/> : <span className={x.ok ? "text-sage" : "text-brick"}>{x.ok ? "✓" : "!"}</span>}<span>{x.name}</span></div><p className="mt-1 text-[10px] leading-4 text-ink/40">{x.status === "checking" ? "Checking…" : (x.detail || (x.ok ? "Check passed." : "Check failed."))}</p></div>)}</div>}</Card></div>
      </>);
      case "overview":
      default:
        return (<>
          <Card className="overflow-hidden"><div className="border-b border-line px-5 py-5"><p className="text-[9px] font-bold uppercase tracking-[.14em] text-ink/30">Control center</p><h2 className="mt-1 font-display text-xl font-semibold">Settings overview</h2><p className="mt-1 max-w-2xl text-xs leading-5 text-ink/45">Settings are grouped by purpose. Select an area from the sidebar and only that panel is rendered, keeping the page simpler and preventing inactive configuration sections from triggering runtime errors.</p></div><div className="grid grid-cols-3 gap-3 p-5">{[["Processing","Operations, features, Client Estimate, Body Analyzer and Global Variables","operations"],["Notifications","Email delivery and notification sounds","email"],["Workspace","Appearance, navigation and connection","appearance"],["Admin access","Admin accounts and database information","database"],["Diagnostics","Live API, queue and worker checks","logs"],["Runtime","Backend runtime configuration","general"]].map(([title,desc,id]) => <button key={id} onClick={() => selectSection(id)} className="rounded-xl border border-line bg-paper/40 p-4 text-left transition hover:border-blueprint/25 hover:bg-blueprint/5"><div className="text-xs font-semibold">{title}</div><p className="mt-1 text-[10px] leading-4 text-ink/40">{desc}</p><span className="mt-3 inline-block text-[9px] font-bold uppercase tracking-wide text-blueprint">Open →</span></button>)}</div></Card>
        </>);
    }
  };
  ...
  return <AppShell>
    <div className="h-full overflow-y-auto bg-paper px-7 py-6">
      <div className="mx-auto max-w-[1400px]">
        ...
        {loading ? <div className="space-y-4"><div className="rounded-2xl border border-line bg-white p-5"><div className="h-4 w-28 animate-pulse rounded bg-ink/[.05]"/><div className="mt-3 h-6 w-64 animate-pulse rounded bg-ink/[.07]"/><div className="mt-2 h-3 w-96 max-w-full animate-pulse rounded bg-ink/[.035]"/></div><div className="grid grid-cols-2 gap-4">{Array.from({length:4}).map((_,i)=><div key={i} className="animate-pulse rounded-2xl border border-line bg-white p-5"><div className="h-4 w-36 rounded bg-ink/[.055]"/><div className="mt-4 h-10 rounded-xl bg-ink/[.035]"/><div className="mt-3 h-10 rounded-xl bg-ink/[.035]"/><div className="mt-3 h-16 rounded-xl bg-ink/[.035]"/></div>)}</div></div> : <div className="min-w-0 space-y-4">
                        {renderActiveSection()}
