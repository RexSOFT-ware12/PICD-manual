    ...
    <div className="h-full overflow-y-auto bg-paper">
      <div className="mx-auto min-h-full max-w-[1280px] px-8 py-7">
        ...
        <section className="mt-6 overflow-hidden rounded-2xl border border-line bg-white shadow-sm">
          ...
          {loading ? <div className="space-y-2 p-4">{Array.from({length:7}).map((_,i)=><div key={i} className="h-20 animate-pulse rounded-xl border border-line bg-ink/[.035]" />)}</div> : filtered.length === 0 ? <div className="px-5 py-20 text-center"><div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-sage/10 text-sage text-xl">✓</div><h3 className="mt-4 font-display text-base font-semibold">Nothing needs attention</h3><p className="mt-1 text-xs text-ink/35">There are no notifications matching this filter.</p></div> : <div className="max-h-[560px] overflow-y-auto divide-y divide-line">
            {filtered.map(a => {
