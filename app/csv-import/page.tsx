...
const STEPS = [
  ["file", "File received", "Reading the uploaded CSV or XLSX file"],
...
export default function CSVImportPage(){
  ...
  const begin=async()=>{if(!file||busy)return;setBusy(true);setError(null);try{const j=await startCSVImport(settings,file);setJob(j);setFile(null);if(inputRef.current)inputRef.current.value="";}catch(e){setError(e instanceof ApiError?e.message:"Could not start import.");}finally{setBusy(false)}};
  ...
  return <AppShell><div className="h-full overflow-hidden bg-paper px-8 py-6"><div className="mx-auto flex h-full min-h-0 max-w-[1400px] flex-col">
    <header className="shrink-0"><p className="text-[10px] font-bold uppercase tracking-[.18em] text-ink/30">Control · Data</p><div className="mt-1 flex items-end justify-between"><div><h1 className="font-display text-3xl font-semibold">CSV importer</h1><p className="mt-1 text-sm text-ink/45">Run the existing backend import flow for CSV or XLSX and watch every database step.</p></div>{job&&<span className={`rounded-full px-3 py-1.5 text-[9px] font-bold uppercase tracking-wider ${job.status==="completed"?"bg-sage/10 text-sage":job.status==="failed"?"bg-brick/10 text-brick":"bg-blueprint/5 text-blueprint"}`}>{job.status}</span>}</div></header>
    ...
    <div className="mt-5 grid min-h-0 flex-1 grid-cols-[390px_1fr] gap-5 overflow-hidden">
      <section className="flex min-h-0 flex-col rounded-2xl border border-line bg-white p-5 shadow-sm"><div><p className="text-[9px] font-bold uppercase tracking-[.15em] text-ink/30">Input file</p><h2 className="mt-1 text-lg font-semibold">Drop a file to import</h2><p className="mt-1 text-xs leading-5 text-ink/40">CSV and XLSX files are supported. The filename determines the MongoDB collection, matching the existing <span className="font-mono">csv_to_db.py</span> behavior.</p></div>
        <label className="mt-5 flex min-h-[190px] cursor-pointer flex-1 items-center justify-center rounded-2xl border border-dashed border-blueprint/20 bg-paper/50 text-center transition hover:bg-paper"><input ref={inputRef} type="file" accept=".csv,.xlsx,text/csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" className="hidden" onChange={e=>{const f=e.target.files?.[0]??null;setError(null);setFile(f)}} name="field_page_1"/><div><div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-blueprint text-xl text-paper">⇅</div><p className="mt-4 text-sm font-semibold">{file?file.name:"Choose CSV or XLSX"}</p><p className="mt-1 text-[10px] text-ink/35">CSV / XLSX · maximum 50 MB</p></div></label>
        ...
        <button onClick={begin} disabled={!file||busy||!!(job&&["queued","running"].includes(job.status))} className="mt-4 rounded-xl bg-blueprint px-4 py-3 text-xs font-semibold text-paper disabled:cursor-not-allowed disabled:opacity-35">{busy?"Starting…":job&&["queued","running"].includes(job.status)?"Import running…":"Run import"}</button>
      </section>
