        ...
        {scan.image_analysis && (() => {
          ...
          return <section className="mt-4 rounded-xl border border-line bg-white/55 p-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[['Front', front], ['Side', side]].map(([label, item]) => { const x = item as typeof front; return <div key={String(label)} className="rounded-lg border border-line/70 bg-paper/60 p-3"><p className="mb-2 text-[10px] font-bold uppercase tracking-wider text-ink/45">{String(label)} image</p><div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[11px]"><Row label="Status" value={x?.status ?? (state === "analyzing" ? "analyzing" : "—")} /><Row label="Resolution" value={x?.resolution ?? "—"} /><Row label="Sharpness" value={value(x?.sharpness)} /><Row label="Brightness" value={value(x?.brightness)} /><Row label="Pose confidence" value={x?.pose_confidence == null ? "—" : `${Math.round(x.pose_confidence * 100)}%`} /><Row label="Gemini review" value={x?.gemini_status ?? "—"} /><Row label="AI confidence" value={x?.gemini_confidence == null ? "—" : `${Math.round(Number(x.gemini_confidence) * 100)}%`} /><Row label="Left / right arm" value={pose(x)} /></div>{x?.gemini_summary && <p className="mt-2 rounded-md bg-blueprint/5 px-2 py-1.5 text-[10px] leading-4 text-ink/55"><span className="font-semibold text-blueprint">Gemini:</span> {x.gemini_summary}</p>}{x?.gemini_issues?.length ? <p className="mt-1 text-[10px] text-amber">{x.gemini_issues[0]}</p> : null}</div> })}
            </div>
        ...
        {scan.result?.global_variables && (
          <div className="mb-4 rounded-xl border border-line bg-white p-4">
            ...
            <div className="grid grid-cols-2 gap-x-5 gap-y-1.5">
              {((scan.result.global_variables && Array.isArray(scan.result.global_variables.Measurements)) ? scan.result.global_variables.Measurements : []).filter(x => x && x.label).map((x, i) => <Row key={`${x.label}-${i}`} label={String(x.label)} value={x.value === null || x.value === undefined ? "—" : String(x.value)} />)}
            </div>
          </div>
        )}
