...
export default function AppShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    try { setSidebarCollapsed(window.localStorage.getItem("picd-sidebar-collapsed") === "1"); } catch {}
  }, []);

  useEffect(() => {
    try { window.localStorage.setItem("picd-sidebar-collapsed", sidebarCollapsed ? "1" : "0"); } catch {}
  }, [sidebarCollapsed]);
  ...
  // Must remain above the auth early-return so hook order never changes.
  const settingsOpen = pathname === "/system" || pathname.startsWith("/system/");
  useEffect(() => {
    if (!settingsOpen) { setActiveSetting("overview"); return; }
    const match = pathname.match(/^\/system\/([^/]+)/);
    setActiveSetting(match?.[1] || "overview");
  ...
  useEffect(() => {
    ...
    const raw = window.sessionStorage.getItem("picd-sidebar-scroll");
    if (raw) {
      const value = Number(raw);
      if (Number.isFinite(value)) window.requestAnimationFrame(() => nav.scrollTop = value);
    }
  }, [pathname]);
  ...
  const rememberNavScroll = () => {
    const nav = navScrollRef.current;
    if (!nav || typeof window === "undefined") return;
    try { window.sessionStorage.setItem("picd-sidebar-scroll", String(nav.scrollTop)); } catch {}
  };
  ...
  return (
    <>
      <div className="mobile-unavailable" role="status" aria-live="polite"><div className="mobile-unavailable-card"><p className="mobile-unavailable-kicker">PICDs</p><h1>Desktop dashboard only</h1><p>This monitoring dashboard is designed for desktop screens and is not available on mobile devices.</p></div></div>
      <div className="desktop-dashboard picd-theme-root flex h-screen min-h-0 overflow-hidden" style={{"--picd-page-bg": pageTheme.background, "--picd-page-surface": pageTheme.surface, "--picd-page-border": pageTheme.border, "--picd-page-text": pageTheme.text, "--picd-page-muted": pageTheme.muted_text, "--picd-page-accent": pageTheme.accent, "--picd-success": pageTheme.success, "--picd-warning": pageTheme.warning, "--picd-error": pageTheme.error, "--picd-sidebar-bg": sidebarTheme.background, "--picd-sidebar-text": sidebarTheme.text, "--picd-sidebar-muted": sidebarTheme.muted_text, "--picd-sidebar-section": sidebarTheme.section_text, "--picd-sidebar-active-bg": sidebarTheme.active_background, "--picd-sidebar-active-text": sidebarTheme.active_text, "--picd-sidebar-hover": sidebarTheme.hover_background, "--picd-sidebar-border": sidebarTheme.border, "--picd-sidebar-width": `${sidebarCollapsed ? 72 : (Number(sidebarTheme.width_px) || 248)}px` } as CSSProperties}>
          ...
          <header className="flex h-[68px] shrink-0 items-center justify-between border-b border-line bg-white/90 px-7 backdrop-blur-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              <div ref={profileRef} className="relative ml-1">
                <button type="button" onClick={() => setProfileOpen(v => !v)} aria-expanded={profileOpen} aria-haspopup="menu" className="flex items-center gap-3 rounded-xl border border-line bg-white px-3 py-2 text-left shadow-sm transition hover:bg-paper">
                  <span className="flex h-8 w-8 items-center justify-center rounded-full bg-blueprint text-[11px] font-bold text-paper">{admin.username.slice(0,1).toUpperCase()}</span>
                  <span className="hidden min-w-0 lg:block"><span className="block max-w-[150px] truncate text-[11px] font-semibold text-ink">{admin.username}</span><span className="block text-[9px] text-ink/40">{admin.is_super_admin ? "Super admin" : "Administrator"}</span></span>
                  <span className="text-[10px] text-ink/35">⌄</span>
                </button>
                {profileOpen && <div role="menu" className="absolute right-0 top-12 z-50 w-64 overflow-hidden rounded-2xl border border-line bg-white shadow-2xl">
                  <div className="border-b border-line bg-paper px-4 py-4"><p className="text-[9px] font-bold uppercase tracking-[.16em] text-ink/35">Signed in</p><p className="mt-1 font-display text-sm font-semibold">{admin.username}</p><p className="mt-1 text-[10px] text-ink/40">{admin.is_super_admin ? "Super admin" : "Administrator"}</p></div>
                  <div className="p-2">
                    <Link href="/profile" onClick={() => { setProfileOpen(false); rememberNavScroll(); }} className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-xs font-medium text-ink/65 hover:bg-ink/[.04]">◯ <span>My profile</span></Link>
                    {can("system.read") && <Link href="/system/general" onClick={() => { setProfileOpen(false); rememberNavScroll(); }} className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-xs font-medium text-ink/65 hover:bg-ink/[.04]">⚙ <span>Connection & Runtime</span></Link>}
                    <button type="button" onClick={signOut} disabled={loggingOut} className="mt-1 flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-xs font-medium text-brick hover:bg-brick/[.05] disabled:opacity-50">↪ <span>{loggingOut ? "Signing out…" : "Sign out"}</span></button>
                  </div>
                </div>}
              </div>
            </div>
